...
p.add_argument(
   ...
   default=None,
   help=(
      "Raw IR code (microseconds) to transmit. "
      "Format: JSON list like '[9000,4500,560,...]' or CSV like '9000,4500,560,...'. "
...
def parse_code_arg(s):
   """Parse --code into a list of int microseconds.

   Accepts either:
   - JSON list: [9000, 4500, 560, ...]
   - CSV / whitespace / semicolon separated: 9000,4500,560 ...
   """
   if s is None:
   ...
   if raw.startswith('['):
      ...
      if not isinstance(v, list) or not v:
         raise ValueError("code must be a non-empty JSON list")
      out = []
      for i in v:
   ...
   # CSV / whitespace / semicolon separated form.
   parts = []
   buf = raw.replace(';', ' ').replace(',', ' ').split()
   parts.extend(buf)
   if not parts:
...
if args.record: # Record.
   ...
   def play_code(label, code_list):
      # Create wave
      marks_wid = {}
      spaces_wid = {}

      wave = [0]*len(code_list)
